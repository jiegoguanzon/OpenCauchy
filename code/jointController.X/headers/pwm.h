...
void pwm3_init();
void pwm_duty_init();
void pwm_disable_outputs();
void pwm_enable_outputs();

void pwm_init () {

    PCLKCON = 0x0000;
    /* Master clock source is selected as High speed PLL clock */
    PCLKCONbits.MCLKSEL = 0x0;

    /* Set Master PWM Phase Register - No phase shift */
    MPHASE = 0x0000;
    /* Set Master Duty Cycle */
    MDC = 0x0000;
    /* Set PWM Period */
    MPER = PWM_FREQ_REG;
...
void pwm_disable_outputs () {
    ...
    PG3IOCONLbits.OVRENH = 1;  // 0 = PWM generator provides data for PWM3H pin
    PG2IOCONLbits.OVRENH = 1;  // 0 = PWM generator provides data for PWM2H pin

...
//        mnemonic.put(0x060000, "return");
    ...
    static String StaticAnalysis(Bl2b bl2file)
    {
        ...
        for(int i = 255; i >= 0; i--)
        {
            ...
            if(p.first == 0)    // Skip printing opcodes that don't appear anywhere in the input
                continue;
        ...
        outLines.add(line++, String.format("\n\n\n"
                                           ...
                                           + "\nTotal compressed if contiguous: %4$,d bytes (%5$,d bytes uncompressed, %6$5.2f%%)",
                                           uniqueOpcodeCount,
                                           uniqueOpcodeCount * 3,
                                           minBitsPerDictionaryWord,
                                           compressedSize,
                                           instrCountTotal * 3,
                                           ((double)compressedSize) / (instrCountTotal * 3) * 100.0
             ));
        ...
        {
            PairWithText p = instanceCountList.get(i);
            outLines.add(line++, String.format("\n  %1$06X %2$-48s: %3$,8d (%4$5.2f%% of total flash, %5$5.2f%% of effective code) %6$,d bytes compressed (%7$,d uncompressed)",
                                               p.second, p.text, p.first, 100.0 * p.first / instrCountTotal, 100.0 * p.first / effectiveInstrCountTotal, 3 + p.first * ((((int)(minBitsPerDictionaryWord + 0.99999)) + 7) / 8), p.first * 3));
        }
        ...
        while(true)
        {
            ...
            for(PCindex = 0; PCindex < instrInPkt; PCindex++)
            {
                ...
                lastWasSkip = (ophigh == 0xA6)
                        ...
                        (ophigh == 0xAF)
                              || // btsc f
                        ((opcode & 0xFF83E0) == 0xE60000) || ((opcode & 0xFF83E0) == 0xE68000)
                              ||// cpsgt/cpslt
                        ((opcode & 0xFF83E0) == 0xE70000) || ((opcode & 0xFF83E0) == 0xE78000);  // cpseq/cpsne
                lastWasMultiCycle = (ophigh == 0x01)
                                    || // bra/call/goto/rcall
                        (ophigh == 0x02)
                                    || // call
                        (ophigh == 0x04)
                ...
                lastWasObviousBranch = (ophigh == 0x01)
                        ...
                        (ophigh == 0x07)
                                       || // rcall
                        (ophigh == 0x37);   // bra
                lastWasCall = ((opcode & 0xFFFFF0) == 0x012000)
                              || // rcall Wn (pic24f)
                        ((opcode & 0xFFFFF0) == 0x010200)
                              || // rcall Wn (dspic33e)
                        ((opcode & 0xFF87F0) == 0x018000)
                              || // call.l Wn+1:Wn
                        ((opcode & 0xFFFFF0) == 0x010000)
                              || // call Wn (pic24f and dspic33e)
                        (ophigh == 0x02)
                              || // call #lit23
                        (ophigh == 0x07);   // rcall (pc relative)
                lastWasGotoBra = ((ophigh == 0x01)
                                  && // bra/call/goto/rcall
                        !(((opcode & 0xFFFFF0) == 0x012000)
                          || // rcall Wn (pic24f)
                        ((opcode & 0xFFFFF0) == 0x010200)
                          || // rcall Wn (dspic33e)
                        ((opcode & 0xFF87F0) == 0x018000)
                ...
                lastWasConditionalBra = (ophigh == 0x0C)
                                        || // bra OA
                        (ophigh == 0x0D)
                                        || // bra OB
                        (ophigh == 0x0E)

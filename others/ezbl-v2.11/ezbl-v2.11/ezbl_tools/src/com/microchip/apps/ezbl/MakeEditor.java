...
public class MakeEditor
{
    ...
    public static void main(String[] args)
    {
        ...
        String linkCommand, originalLinkOutputFile;
        ...
        while(makeData.Find("(?m)^[\t][^\\$]*[\\$]([{]|[(])MP_CC([}]|[)]).*?-Wl[^\n]*", "\n"))
        {
            ...
            if(state.dualPartitionMode != partitionOptionFound)
            {
                ...
                if(linkCommand.contains("--partition,1"))
                    state.targetPartition = 1;
                else if(linkCommand.contains("--partition,2"))
                    state.targetPartition = 2;
            }
            if(!partitionOptionFound)
                state.targetPartition = 0;
            ...
            state.saveTemps |= linkCommand.toLowerCase().contains("-save-temps");
            ...
            String firstLinkCommand = linkCommand.replaceAll("[\\s]+-o[\\s]+[\\S]+[\\s]", Matcher.quoteReplacement(" -o \"" + state.elfPath + "\" ")).replaceAll("--check-sections", Matcher.quoteReplacement("--no-check-sections")) + " 1>" + EZBLState.nullFile();
            ...
            String dump1Command = state.javaLaunchString + " --dump_parser -pass=1 -elf_artifact=\"" + state.elfPath + "\" " + expandedPassThroughOptions;
            String dump2Command = state.javaLaunchString + " --dump_parser -pass=2 -elf_artifact=\"" + originalLinkOutputFile + "\" " + expandedPassThroughOptions;
            // NOTE: the following line IS a --dump_parser -pass=2 command line, not an echo or linking command. The echo output is piped to --dump_parser to pass the exact command line needed for final linking in case if a 3rd linking pass is needed. The DumpParser class will auto-launch the linker for this step when needed.
            dump2Command = "@echo " + dump2Command + "\n\t"
                           + "@echo " + linkCommand + " | " + dump2Command;
            String genMergeCommand = state.javaLaunchString + " --blobber -generate_merge " + expandedPassThroughOptions.replace("-artifact=\"$(FINAL_IMAGE)\"", "-artifact=\"" + originalLinkOutputFile + "\"");

            String newLink1Command = "###<EZBL MODIFIED 0>###\n";
            if(state.MCU16Mode)
            {
                newLink1Command += "\t@echo EZBL: Updating automatic sections in linker script\n"
                                   + "\t" + state.javaLaunchString + " --gldbuilder " + expandedPassThroughOptions + "\n";
            }
            newLink1Command += "\t@echo EZBL: Starting linking pass 1 of 2\n"
                               + firstLinkCommand + "\n";
            ...
            if(!state.saveTemps)    // Delete ezbl_partial_link.elf used in pass 1 linking if save-temps not specified in MPLAB project and the file exists (wasn't already deleted)
                newCommands.add("\t@test -e \"" + state.elfPath + "\" && rm \"" + state.elfPath + "\" || true\n");
            newCommands.add("###</EZBL MODIFIED 0>###\n");
        ...
        // Add --blobber commands for projects with Loadables/.unified.hex output
        // Pre generate commands to generate merge.s/merge.gld files
        newCommands = new ArrayList<>();
        ...
        str = Multifunction.CatStringList(newCommands);
        makeData.ResetFind();
        while(makeData.Find("(?m)^[\t][^\n]*?hexmate[\\s\".]*", "-[oO][\"]?", "[\\s\"]", "\n"))         // Replace multiple invocations of hexmate
        {

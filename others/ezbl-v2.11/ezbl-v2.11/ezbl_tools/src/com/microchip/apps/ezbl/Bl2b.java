...
// Do not serialize to get the .bl2 file normally. Use getBytes() (won't include encryption)
{
     ...
     *                           including synchronization string, all headers
     *                           and hash + CRC32
     ...
     * from the original .blob file format, which is not the planned encryption
     * method.
     *
     * @param state
     * @param bl2Bytes
    ...
    {
        long startAddr = architecture16Bit ? range.startAddr : range.startAddr & 0x1FFFFFFFL;   // Use Physical addresses for PIC32MM
        long endAddr = architecture16Bit ? range.endAddr : range.endAddr & 0x1FFFFFFFL;

        // Create a temporary blob containing only filler
        DataRecord newData = new DataRecord(architecture16Bit);
        newData.address = startAddr;
        newData.data = new byte[newData.getDataIndexOfAddress(endAddr)];
        ...
        {
            DataRecord rec = this.records.get(i);
            if((rec.address >= endAddr) || (rec.getEndAddress() <= startAddr))    // No interference
                continue;
     ...
     *                         beginning of the file, typically data containing
     *                         auto-baud synchronization and/or other bootloader
     *                         wake up data. If this parameter is null, the
     *                         default synchronization header string of
     *                         "UUUUUUUUMCUPHCME" is generated. If this
     *                         parameter is an empty string, no synchronization
     *                         header data is prepended.
     *
     ...
     *
     * @return String representing the decoded data contents in human readable
     *         form, highlighting any errors detected in the data.
     */
    ...
    /**
     * Returns a human readable text representation of a .hex file's contents.
     *
     * Useful for debugging and easily checking what's in a .hex file
     *
    ...
    /**
     * Returns a human readable text representation of a .bl2 file's contents.
     *
     * Useful for debugging, .bl2 format analysis, or learning about the .bl2
     * structure for implementation in another tool.
     *

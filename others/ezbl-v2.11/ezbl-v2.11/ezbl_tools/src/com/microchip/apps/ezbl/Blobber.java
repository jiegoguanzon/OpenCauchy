...
public class Blobber
{
    ...
    public static void main(String[] args)
    {
        ...
        for(String arg : args)
        {
            ...
            else if(arg.toLowerCase().matches("(--help)|(-help)|(/\\?)|(-h)|(--h)"))
                showHelp = true;
        }
            ...
            else if(lowerToken.startsWith("-void="))    // Nuke all data within the address range from the output file. No records will have data defined within the address range(s).
            {
                ...
                for(String param : params)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ar.startAddr = Long.decode(TrimQuotes(param));
                        }
                        else
                        {
                            ar.endAddr = Long.decode(TrimQuotes(param));
                            if(ar.startAddr != (long)-1)
                            {
                                voidRanges.add(ar);
                    ...
                    try
                    {
                        switch(i % 0x3)
                        {
                            ...
                            default:    // Fill byte value
                                ...
                                if((ar.startAddr != (long)-1) && (ar.endAddr != (long)-1))
                                {
                                    fillRanges.add(ar);
                                    ar = new AddressRange((long)-1, (long)-1);
        ...
        // If artifact is an .elf, dump symbols to get BOOTID_HASH/APPID_VER and convert to .hex to ensure we get the right flash data (if no .hex already available)
        if((state.artifactPath != null) && state.artifactPath.matches(".*?[.][eE][lL][fF]$"))
        {
            ...
            if(metaParser.startObjDump(cmdLine) < 0)
            {
                ...
                if(!state.silent)
                {
                    System.err.printf("    \"" + CatStringList(cmdLine, " ") + "\"\n"
                                      + "    Ensure %s's bin folder is in the system path, set via the -compiler_folder= command line option, or assigned\n"
                                      + "    using the MP_CC_PATH environment variable.\n", toolPrefix.toUpperCase());
                    System.exit(elfMachine == e_machine.EM_PIC30 ? -16 : ((elfMachine == e_machine.EM_MIPS) ? -32 : -1632));
                }
            }
            if(state.MCU16Mode && (metaParser != null))
            {
                cmdLine.remove("--syms");
        ...
        if(metaParser != null)
        {
            ...
            metaParser.parseAllObjOutput(0);    // Get Headers and Symbols (also section data with XC32)
            metaParser.parseAllObjOutput(0);    // Get Section data (for XC16 only)
            state.elfDump = metaParser;

            sec = metaParser.romSectionMapByName.get(".text.EZBL_bootIDHash");
            if((sec != null) && (sec.data != null) && (sec.data.data != null))
                bl2.bootIDHash = Multifunction.BytesToInt32s(sec.data.data, 0, sec.data.data.length);
            else
            {
                s = metaParser.symbolsByName.get("BOOTID_HASH0");
                if(s != null)
                {
                    bl2.bootIDHash[0] = (int)s.address;
                    bl2.bootIDHash[1] = (int)metaParser.symbolsByName.get("BOOTID_HASH1").address;
                    bl2.bootIDHash[2] = (int)metaParser.symbolsByName.get("BOOTID_HASH2").address;
                    bl2.bootIDHash[3] = (int)metaParser.symbolsByName.get("BOOTID_HASH3").address;
                }
                else
                {
                    s = metaParser.symbolsByName.get("_BOOTID_HASH0");
                    if(s != null)
                    {
                        bl2.bootIDHash[0] = (int)s.address;
                        bl2.bootIDHash[1] = (int)metaParser.symbolsByName.get("_BOOTID_HASH1").address;
                        bl2.bootIDHash[2] = (int)metaParser.symbolsByName.get("_BOOTID_HASH2").address;
                        bl2.bootIDHash[3] = (int)metaParser.symbolsByName.get("_BOOTID_HASH3").address;
                    }
                }
            }

            s = metaParser.symbolsByName.get("APPID_VER_MAJOR");
            if(s != null)
            {
                bl2.appIDVerMajor = (int)s.address;
                bl2.appIDVerMinor = (int)metaParser.symbolsByName.get("APPID_VER_MINOR").address;
                bl2.appIDVerBuild = (int)metaParser.symbolsByName.get("APPID_VER_BUILD").address;
            }
            else
            {
                s = metaParser.symbolsByName.get("_APPID_VER_MAJOR");
                if(s != null)
                {
                    bl2.appIDVerMajor = (int)s.address;
                    bl2.appIDVerMinor = (int)metaParser.symbolsByName.get("_APPID_VER_MINOR").address;
                    bl2.appIDVerBuild = (int)metaParser.symbolsByName.get("_APPID_VER_BUILD").address;
                }
            }
        }
        ...
        String outputMergeLinkerExt = state.MCU16Mode ? ".gld" : ".ld";

        // Optimize records
        bl2.Coalesce(true, state.hexAlign, state.hexAlign);
        ...
        if(state.ignoreROMRegions != null)
        {
            bl2.RemoveIgnoreRanges(state.ignoreROMRegions);
            bl2.Coalesce(true, state.hexAlign, state.hexAlign);
        }
        ...
        for(int i = 0; i < fillRanges.size(); i++)
        {
            ...
            DataRecord.ExtractRange(bl2.records, ar.startAddr, ar.endAddr);  // Discards everything overlapping
            DataRecord dr = new DataRecord();
            dr.address = ar.startAddr;
            dr.architecture16Bit = bl2.architecture16Bit;
            int fillSize = bl2.architecture16Bit ? (int)(ar.endAddr - ar.startAddr) * 3 / 2 : (int)(ar.endAddr - ar.startAddr);
        ...
        for(int i = 0; i < padRanges.size(); i++)
        {
            ...
            List<DataRecord> drs = DataRecord.ExtractRanges(bl2.records, ar.startAddr, ar.endAddr);
            if((drs == null) || (drs.isEmpty()))
            {   // No data existed in the given range, just create a new data range as like fill data
                DataRecord dr = new DataRecord();
                dr.address = ar.startAddr;
                dr.architecture16Bit = bl2.architecture16Bit;
                int padSize = bl2.architecture16Bit ? (int)(ar.endAddr - ar.startAddr) * 3 / 2 : (int)(ar.endAddr - ar.startAddr);
            ...
            {   // Extend existing records with padding on left side, except for the last record, which gets padding on both the left and right sides, as applicable
                ...
                int leftPadSize = dr.architecture16Bit ? (int)((dr.address - ar.startAddr) * 3 / 2) : (int)(dr.address - ar.startAddr);
                int rightPadSize = 0;
                if(drIndex == drs.size() - 1)
                {
                    rightPadSize = dr.architecture16Bit ? (int)((ar.endAddr - dr.getEndAddress()) * 3 / 2) : (int)(ar.endAddr - dr.getEndAddress());
                }
        ...
        {
            DataRecord.ExtractRange(bl2.records, ar.startAddr, ar.endAddr);
        }
        ...
        for(int i = 0; i < movRanges.size(); i++)
        {
            ...
            {
                dr.address -= ar.startAddr - movDestinations.get(i);
                bl2.records.add(dr);
            }
        }
        ...
        if(addPhantoms && (bl2.records != null) && !bl2.records.isEmpty() && bl2.records.get(0).architecture16Bit)
        {
            DataRecord.CoalesceRecords(bl2.records, true, 0x2, 0x2);
            for(DataRecord dr : bl2.records)
            {
                byte[] phantomPaddedData = new byte[dr.data.length / 3 * 4];
                int phantomIndex = 0;
                for(int i = 0; i < dr.data.length; i += 3)
                {
                    phantomPaddedData[phantomIndex++] = dr.data[i + 0];
                    phantomPaddedData[phantomIndex++] = dr.data[i + 1];
                    phantomPaddedData[phantomIndex++] = dr.data[i + 2];
                    phantomIndex++;
                }
                dr.data = phantomPaddedData;
                dr.architecture16Bit = false;
                dr.address *= 2;
            }
        }

        if(rowCompressed && (bl2.records != null) && !bl2.records.isEmpty() && bl2.records.get(0).architecture16Bit)
        {
            DataRecord.CoalesceRecords(bl2.records, true, 0x4, 0x4);
            for(DataRecord dr : bl2.records)
            {
                for(int i = 0; i < dr.data.length; i += 6)
                {
                    byte b = dr.data[i + 5];
                    dr.data[i + 5] = dr.data[i + 4];
                    dr.data[i + 4] = dr.data[i + 3];
                    dr.data[i + 3] = b;
                }
            }
        }

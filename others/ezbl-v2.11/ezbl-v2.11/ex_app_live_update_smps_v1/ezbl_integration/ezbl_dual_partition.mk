...
# Post-build code to generate .bl2 file from .elf, attempt upload to bootloader, and copy generated .elf file to subsequent "Live Update" project folder for future use
ezbl_post_build: .build-impl
#	Copy this v1 .elf artifact to the v2 base project folder so they can do v1->v2 live update builds with the --preserved linker option
	-test -d "../ex_app_live_update_smps_v2" && test "../ex_app_live_update_smps_v2/${PROJECTNAME}.${IMAGE_TYPE}.${DEBUGGABLE_SUFFIX}" -ot "${DISTDIR}/${PROJECTNAME}.${IMAGE_TYPE}.${DEBUGGABLE_SUFFIX}" && cp "${DISTDIR}/${PROJECTNAME}.${IMAGE_TYPE}.${DEBUGGABLE_SUFFIX}" "../ex_app_live_update_smps_v2/${PROJECTNAME}.${IMAGE_TYPE}.${DEBUGGABLE_SUFFIX}" || true
